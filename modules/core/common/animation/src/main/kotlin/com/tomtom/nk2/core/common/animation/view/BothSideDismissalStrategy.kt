...
class BothSideDismissalStrategy(context: Context, dismissViewWidthThreshold: Float) :
    ...
    override fun getDismissDirection(distancePx: Float, thresholdPx: Float): DismissDirection? =
        when {
            distancePx < -thresholdPx -> DismissDirection.LEFT
            distancePx > thresholdPx -> DismissDirection.RIGHT
            else -> null
        }

    override fun getMovableArea(view: View) = -view.width.toFloat()..view.width.toFloat()
}
